...
**ALL development work MUST follow Test-Driven Development (TDD)** - see [tdd-methodology.md](mdc:.roo/rules/tdd-methodology.md) for complete guidelines.

### **TDD Integration with Task Master:**
1. **Get Task**: `task-master show <id>` - understand requirements
2. **RED Phase**: Write failing tests first
   - Create comprehensive test suite covering all requirements
   - Commit tests: `git commit -m "test: add failing tests for task <id>"`
3. **GREEN Phase**: Implement minimal code to pass tests
   - Write only enough code to make tests pass
   - Commit implementation: `git commit -m "feat: implement task <id>"`
4. **REFACTOR Phase**: Improve code quality
   - Optimize and clean up while keeping tests green
   - Commit refactoring: `git commit -m "refactor: optimize task <id>"`
5. **Mark Complete**: `task-master set-status --id=<id> --status=done`

### **Frequent Commit Requirements:**
- ✅ **Every TDD phase**: RED, GREEN, REFACTOR commits
- ✅ **Every test file**: Immediate commit when creating tests
- ✅ **Every feature**: Multiple commits throughout TDD cycle
- ✅ **Every bug fix**: Test reproduction + fix commits
...
## TDD-Enhanced Development Workflow Process

-   **Start TDD cycle** for new projects by running `initialize_project` tool / `task-master init` or `parse_prd` / `task-master parse-prd --input='<prd-file.txt>'` (see [`taskmaster.md`](mdc:.roo/rules/taskmaster.md)) to generate initial tasks.json
...
-   **MANDATORY TDD**: Before any implementation, write comprehensive failing tests
...
-   **TDD Implementation**: Follow RED-GREEN-REFACTOR cycle for all code changes
-   Verify tasks according to test strategies before marking as complete (See [`tests.md`](mdc:.roo/rules/tests.md))
-   **Frequent commits**: After each TDD phase and significant progress
